<div class="row">
    ...
    <div class="col-sm-12 col-md-5">
        <?=form_open_multipart('cuti/form/approval?'.$_SERVER['QUERY_STRING'], ['class'=>'form', 'id'=>'form_approval_cuti'])?>
            <div class="card mb-2">
                <div class="card-header color-green">
                    <?=(isset($title))?$title:'Pejabat yang memberikan cuti'?>
                </div>
                <div class="card-body" style="background-color: var(--abu-terang);">
                    ...
                    <div class="row mb-2">
                        ...
                        <div class="col-sm-12 col-md-9 fw-bold">
                            <?php foreach (return_referensi_list('cuti_status_approval') as $key) if($key->ref_code>1){?>
                                <label class="m-1"><input type="radio" name="status_approval" onclick="check_status_pengajuan()" value="<?=$key->ref_code?>"> <?=$key->ref_name?></label>
                            <?php }?>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-sm-12 col-md-3">
                            <b class="d-block">Passphrase*:</b>
                        </div>
                        <div class="col-sm-12 col-md-9">
                            <div class="input-group">
                                <input type="password" name="passphrase" id="passphrase" placeholder="Passphrase" class="form-control">
                                <span class="input-group-text">
                                    <i id="iPassword" class="fa fa-eye" onclick="showHidePassword('#passphrase', '#iPassword')"></i>
                                    <i class="fa fa-question-circle text-warning ms-2" data-bs-toggle="tooltip" data-bs-placement="top" title="Passphrase/Password Tanda Tangan Elektronik (TTE)"></i>
                                </span>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card-footer text-center">
                    <button type="submit" class="btn btn-lg btn-warning btn-simpan" data-bs-toggle="tooltip" data-bs-placement="top" title="Proses permohonan"><i class="fa fa-check-circle"></i></button>
                    <?php if(return_access_link(['cuti/permohonan'])){?>
                        <a href="<?=site_url('cuti/permohonan')?>" class="btn btn-lg btn-secondary btn-batal" data-bs-toggle="tooltip" data-bs-placement="top" title="Kembali ke halaman sebelumnya"><i class="fa fa-arrow-alt-circle-left"></i></a>
                    <?php }?>
                </div>
            </div>
        <?=form_close()?>
...
<script type="text/javascript">
    ...
    $('#form_approval_cuti').on('submit', function(e){
        ...
        // {
            ...
            $.ajax({
                ...
                success: function(responseData, textStatus, jqXHR) {
                    ...
                    if(dt.status==true)
                    {
                        window.location.assign('<?=site_url('cuti/permohonan')?>')
                    }else{
                        alert(dt.message)
        ...
        // }else{
        //     showModalInfo('Passphrase tidak boleh kosong.', 1)
        // }
    });
</script>