...
$fileName = 'dataReportPegawaiNonPNS-'.time().'.xlsx';
...
$objPHPExcel->getActiveSheet()->SetCellValue('A1', 'DAFTAR TENAGA NON PNS SEKRETARIAT KANTOR STAF PRESIDEN');
...
$objPHPExcel->getActiveSheet()->SetCellValue('A5', 'TENAGA PENGAMANAN');
...
$no1 = 0;
$n1 = 0;
foreach ($data as $element) if($element->jabatan_id==48){
	$n1 = ++$n1;
...
$objPHPExcel->getActiveSheet()->SetCellValue('A'.($rowCountKS), 'TENAGA PENGEMUDI');
$objPHPExcel->getActiveSheet()->getStyle('A'.($rowCountKS))->applyFromArray($styleArray2);

$rowCountKS = $rowCountKS+1;
$no1 = 0;
$n1 = $n1;
foreach ($data as $element) if($element->jabatan_id==47){
	$n1 = ++$n1;
	if(array_keys(['', '-'],$element->akhir)){
		$tglakhir = '';
	}else{
...
$objPHPExcel->setActiveSheetIndex(0)->mergeCells('A'.($rowCountKS).':H'.($rowCountKS).'');
$objPHPExcel->getActiveSheet()->SetCellValue('A'.($rowCountKS), 'TENAGA PRAMUBAKTI');
$objPHPExcel->getActiveSheet()->getStyle('A'.($rowCountKS))->applyFromArray($styleArray2);

$rowCountKS = $rowCountKS+1;
$no1 = 0;
$n1 = $n1;
foreach ($data as $element) if($element->jabatan_id==40){
	$n1 = ++$n1;
	if(array_keys(['', '-'],$element->akhir)){
		$tglakhir = '';
	}else{
